...
def aggregate_issues_by_workload(
    ...
    workload_kinds = {
        ...
        "Job", "CronJob", "DeploymentConfig"  # DeploymentConfig for OpenShift
    }
    ...
    if trivy_results:
        ...
        for key in workload_issues.keys():
            ...
            if namespace in namespace_vulns:
                ...
                matching_pods = [
                    pod for pod in namespace_vulns[namespace]["pods"]
    ...
    deduplicated_issues = {}
    replicaset_signatures = {}  # Track ReplicaSet issues by namespace to find duplicates
    ...
    if verbosity > 1:
        ...
        if original_count != final_count:
            print(f"[DEBUG] Deduplicated {original_count - final_count} ReplicaSets ({final_count} workloads remaining)")
...
def _set_cell_background(cell, color_rgb: Tuple[int, int, int]):
    """
...
def generate_word_summary_table(
    summary_data: Dict[Tuple[str, str, str], Dict[str, Any]],
    output_path: str,
    verbosity: int = 1
) -> None:
    """
    ...
    sorted_workloads = sorted(
        summary_data.items(), 
        key=lambda x: (str(x[0][0] or ""), str(x[0][1] or ""), str(x[0][2] or ""))
    )
    ...
    stats.add_run(
        f"Summary: Found {total_issues} total issues across {len(summary_data)} workloads."
    ).italic = True
